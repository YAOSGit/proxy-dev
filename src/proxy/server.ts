...
const getMockVariant = (routeKey: string, route: RouteWithMock): MockVariant | null | undefined => {
...
const startServer = (certs: CertPathsForWorker): void => {
    ...
    const getContext = (domain: string): tls.SecureContext | null => {
        ...
        if (contexts.has(domain)) return contexts.get(domain)!;
    ...
    try {
        ...
        if (firstCert) {
            ...
            defaultCert = fs.readFileSync(path.join(certs.leavesDir, `${domain}.crt`));
        }
    } catch (e) {
    ...
    const sslOptions: https.ServerOptions = {
        ...
        SNICallback: (servername, cb) => {
            ...
            if (!ctx) {
                console.warn(`[SNI] No certificate matched for "${servername || 'unknown'}"`);
            }
    ...
    const requestHandler = (req: http.IncomingMessage, res: http.ServerResponse, isHttps: boolean) => {
        const host = req.headers.host?.split(':')[0] ?? '';
        const urlPath = new URL(req.url ?? '/', `http${isHttps ? 's' : ''}://${host}`).pathname;
        ...
        if (mockVariant !== undefined && mockVariant !== null) {
            ...
            const latencyMs = resolveLatency(route.latencyMs, state.latency, mockVariant.latencyMs);
            ...
            const doProxy = () => {
                ...
                proxyReq.on('error', (err) => {
                    ...
                    const entry = buildTrafficEntry(req, 502, Date.now() - startTime, 'LIVE', requestBody);
                    emitEvent({ type: 'request', entry });
                });
            ...
            if (latencyMs > 0) {
                applyLatency(latencyMs).then(doProxy).catch((err) => {
    ...
    httpsServer = https.createServer(sslOptions, (req, res) => requestHandler(req, res, true));
    ...
    httpsServer.on('error', (err: NodeJS.ErrnoException) => {
        if (err.code === 'EADDRINUSE') {
            emitEvent({ type: 'error', message: `Port ${state.port} already in use. Stop the conflicting service or use a different port.` });
        } else if (err.code === 'EACCES') {
            emitEvent({ type: 'error', message: `Permission denied for port ${state.port}. Ports below 1024 require root privileges.` });
        } else {
