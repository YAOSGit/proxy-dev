...
const isProxyReady = (event: ProxyEvent): event is Extract<ProxyEvent, { type: 'ready' }> => {
    return event.type === 'ready';
};

const isProxyRequest = (event: ProxyEvent): event is Extract<ProxyEvent, { type: 'request' }> => {
    return event.type === 'request';
};

const isProxyError = (event: ProxyEvent): event is Extract<ProxyEvent, { type: 'error' }> => {
