...
const clearConfigWarnings = (): void => { configWarnings.length = 0; };
...
const atomicWrite = (filePath: string, content: string): void => {
    ...
    const tmpPath = path.join(os.tmpdir(), `proxy-dev-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
const saveGlobalConfig = (config: GlobalConfig): void => {
    ...
    if (!result.success) {
        console.error(`[config] Refusing to save invalid global config: ${result.error.message}`);
        return;
    }
    const configPath = getGlobalConfigPath();
...
const saveLocalConfig = (config: LocalConfig, cwd?: string): void => {
    ...
    if (!result.success) {
        console.error(`[config] Refusing to save invalid local config: ${result.error.message}`);
        return;
    }
    const dir = cwd ?? process.cwd();
    const configPath = path.join(dir, LOCAL_CONFIG_FILENAME);
...
const resolveRoutes = (global: GlobalConfig, local: LocalConfig | null): ResolvedRoute[] => {
...
const mergeConfigs = (global: GlobalConfig, local: LocalConfig | null): ResolvedConfig => {
...
const loadConfigByMode = (mode: ConfigMode, cwd?: string): ConfigByModeResult => {
    if (mode === 'global') {
