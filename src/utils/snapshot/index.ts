...
const buildMockFilePath = (domain: string, urlPath: string, variantName: string): string => {
    const pathSegments = urlPath.split('/').filter(s => s !== '' && s !== '..' && s !== '.');
    const filePath = path.join(MOCKS_DIR, domain, ...pathSegments, `${variantName}.json`);
...
const writeMockFile = (
    ...
    if (Buffer.byteLength(body, 'utf-8') > MAX_SNAPSHOT_BYTES) {
        throw new Error(`Response body exceeds ${MAX_SNAPSHOT_BYTES / 1024 / 1024}MB snapshot limit`);
    }
...
const addMockToLocalConfig = (
    ...
    mockRoute.variants[variantName] = variant;
