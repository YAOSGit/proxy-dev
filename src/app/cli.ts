...
import { bootstrapGlobalConfig, loadGlobalConfig, loadLocalConfig, resolveRoutes, saveGlobalConfig, saveLocalConfig } from '../utils/config/index.js';
import { getCertsDir, getPidPath } from '../utils/platform/index.js';
import {
    generateCA, loadCA, getTrustCommand, checkTrustStatus, checkFirefoxTrust,
    getFirefoxProfilesDir,
    trustNode, checkNodeTrust, trustPython, checkPythonTrust,
    trustJava, checkJavaTrust, trustOpenssl, checkOpensslTrust,
    trustDeno, checkDenoTrust,
    isRuntimeInstalled, getShellProfile,
} from '../ssl/index.js';
...
const trustCmd = program.command('trust').description('Manage CA trust for system and runtimes');
...
trustCmd
    ...
    .action(() => {
        ...
        try {
            ...
            console.error('Failed to trust CA. You may need to provide sudo password.');
            process.exit(1);
        }
        process.exit(0);
    });
...
trustCmd
    .command('python')
    .description('Configure Python requests/certifi to trust the CA (REQUESTS_CA_BUNDLE)')
    .action(() => {
        const certsDir = getCertsDir();
        const ca = loadCA(certsDir);
        if (!ca) {
        ...
        try {
            ...
            if (result === 'exists') {
                ...
                console.log('✓ Generated combined CA bundle at ~/.config/proxy-dev/certs/combined-ca.pem');
...
trustCmd
    ...
    .action(() => {
        ...
        try {
            ...
            console.error('Failed to import CA. Ensure keytool is on your PATH and provide sudo password.');
            process.exit(1);
        }
        process.exit(0);
    });
...
trustCmd
    .command('openssl')
    .description('Configure SSL_CERT_FILE for OpenSSL-based runtimes (Ruby, Go, Rust, PHP)')
    .action(() => {
        const certsDir = getCertsDir();
        const ca = loadCA(certsDir);
        if (!ca) {
...
trustCmd
    ...
    .action(() => {
        ...
        console.log('');
        console.log(checkTrustStatus(ca.certPath) ? ok('System keychain') : no('System keychain'));
        console.log(checkFirefoxTrust() ? ok('Firefox') : no('Firefox'));
        console.log(isRuntimeInstalled('node') ? (checkNodeTrust() ? ok('Node.js') : no('Node.js')) : skip('Node.js (not installed)'));
        console.log(isRuntimeInstalled('python3') || isRuntimeInstalled('python') ? (checkPythonTrust() ? ok('Python') : no('Python')) : skip('Python (not installed)'));
        console.log(isRuntimeInstalled('java') ? (checkJavaTrust() ? ok('Java') : no('Java')) : skip('Java (not installed)'));
        console.log(checkOpensslTrust() ? ok('OpenSSL (Ruby, Go, Rust, PHP)') : no('OpenSSL (Ruby, Go, Rust, PHP)'));
        console.log(isRuntimeInstalled('deno') ? (checkDenoTrust() ? ok('Deno') : no('Deno')) : skip('Deno (not installed)'));
        console.log('');
        process.exit(0);
    });
...
const daemonCmd = program.command('daemon').description('Manage the hosts daemon');
...
daemonCmd
	...
	.action(async () => {
		const { DaemonClient, isDaemonRunning } = await import('../daemon/index.js');
		const { getDaemonSocketPath } = await import('../utils/platform/index.js');
...
routesCmd
    ...
    .action((domain: string, port: string, opts: { group: string; path?: string }) => {
        const global = loadGlobalConfig();
...
groupsCmd
    ...
    .action((name: string) => {
        ...
        local.activeGroups = [...(local.activeGroups ?? []).filter((g) => g !== name), name];
...
const isMain = resolvedMain === selfPath ||
