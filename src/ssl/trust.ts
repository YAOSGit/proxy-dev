...
const appendWindowsEnv = (exportLine: string): 'added' | 'exists' => {
    ...
    if (process.env[envVar]?.includes('proxy-dev') || process.env[envVar] === value) {
        return 'exists';
    }
...
const getTrustCommand = (caCertPath: string): { command: string; args: string[] } => {
    switch (process.platform) {
        case 'darwin':
            return {
                command: 'sudo',
                args: ['security', 'add-trusted-cert', '-d', '-r', 'trustRoot', '-k', '/Library/Keychains/System.keychain', caCertPath],
            };
        case 'linux': {
            if (fs.existsSync('/etc/pki/ca-trust/source/anchors')) {
                ...
                return {
                    command: 'sudo',
                    args: ['bash', '-c', `cp "${caCertPath}" /etc/pki/ca-trust/source/anchors/proxy-dev-ca.crt && update-ca-trust`],
                };
            }
            if (fs.existsSync('/etc/ca-certificates/trust-source/anchors')) {
                ...
                return {
                    command: 'sudo',
                    args: ['bash', '-c', `cp "${caCertPath}" /etc/ca-certificates/trust-source/anchors/proxy-dev-ca.crt && trust extract-compat`],
                };
            }
            // Default: Debian, Ubuntu, Alpine
            return {
                command: 'sudo',
                args: ['bash', '-c', `cp "${caCertPath}" /usr/local/share/ca-certificates/proxy-dev-ca.crt && update-ca-certificates`],
            };
        }
        case 'win32':
            return {
        ...
        default:
            throw new Error(`Unsupported platform: ${process.platform}. Please add "${caCertPath}" to your system trust store manually.`);
    }
};
...
const checkTrustStatus = (caCertPath: string): boolean => {
    ...
    try {
        switch (process.platform) {
            ...
            case 'linux': {
                execFileSync('openssl', ['verify', '-CApath', '/etc/ssl/certs', caCertPath], {
                    stdio: ['pipe', 'pipe', 'pipe'],
                });
                return true;
            }
...
const getFirefoxProfilesDir = (): string => {
    switch (process.platform) {
        ...
        case 'win32':
            return path.join(process.env.APPDATA ?? os.homedir(), 'Mozilla/Firefox/Profiles');
        default: // linux
...
const trustNode = (caCertPath: string): 'added' | 'exists' => {
    return appendToShellProfile(`export NODE_EXTRA_CA_CERTS="${caCertPath}" # proxy-dev`);
};
...
const trustPython = (caCertPath: string, certsDir: string): 'added' | 'exists' => {
    ...
    return appendToShellProfile(`export REQUESTS_CA_BUNDLE="${combinedPath}" # proxy-dev`);
};
...
const checkJavaTrust = (): boolean => {
    try {
        execFileSync('keytool', ['-list', '-alias', 'proxy-dev', '-cacerts', '-storepass', 'changeit'], {
            stdio: 'pipe',
        });
        return true;
    } catch {
...
const trustOpenssl = (caCertPath: string, certsDir: string): 'added' | 'exists' => {
    ...
    return appendToShellProfile(`export SSL_CERT_FILE="${combinedPath}" # proxy-dev`);
};
