...
const generateLeaf = (
	...
	if (!/^[a-zA-Z0-9._-]+$/.test(domain)) {
		throw new Error(
			'Invalid domain: must contain only alphanumeric characters, dots, hyphens, and underscores',
		);
	}
	...
	try {
		...
		throw new Error(
			'Failed to generate leaf private key for ' +
				domain +
				': ' +
...
`;
	...
	try {
		...
		throw new Error(
			'Failed to sign leaf certificate for ' +
				domain +
				': ' +
				(err instanceof Error ? err.message : String(err)) +
				'\nEnsure openssl is installed and available on your PATH.',
