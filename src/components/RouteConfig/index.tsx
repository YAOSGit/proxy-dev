...
import type { AddGroupFormProps, AddRouteFormProps, RouteConfigProps } from './RouteConfig.types.js';
...
function AddRouteForm({ groupName, onSave, onCancel }: AddRouteFormProps) {
    ...
    const [step, setStep] = useState<'domain' | 'path' | 'target' | 'upgrade'>('domain');
    ...
    useInput((input, key) => {
        ...
        if (key.return) {
            ...
            if (step === 'domain') { setDomain((p) => p.slice(0, -1)); setDomainError(''); }
            else if (step === 'path') setPath((p) => p.slice(0, -1));
            else if (step === 'target') setTarget((p) => p.slice(0, -1));
        } else if (input && !key.ctrl && !key.meta) {
            if (step === 'domain') { setDomain((p) => p + input); setDomainError(''); }
            else if (step === 'path') setPath((p) => p + input);
            else if (step === 'target' && /^\d$/.test(input)) setTarget((p) => p + input);
            else if (step === 'upgrade') {
    ...
    return (
        <Box flexDirection="column" paddingX={2} paddingY={1} borderStyle="round">
            <Text bold color="cyan">Add Route to {groupName}</Text>
            <Box flexDirection="column">
                <Box>
                    <Text color={step === 'domain' ? 'white' : 'gray'}>Domain: </Text>
                    <Text>{domain}{step === 'domain' && <Text color="cyan">|</Text>}</Text>
                </Box>
                {domainError && step === 'domain' && <Text color="red">  {domainError}</Text>}
            </Box>
            {(step !== 'domain') && <Box>
                <Text color={step === 'path' ? 'white' : 'gray'}>Path prefix (optional): </Text>
                <Text>{path}{step === 'path' && <Text color="cyan">|</Text>}</Text>
            </Box>}
            {(step === 'target' || step === 'upgrade') && <Box>
                <Text color={step === 'target' ? 'white' : 'gray'}>Target port: </Text>
                <Text>{target}{step === 'target' && <Text color="cyan">|</Text>}</Text>
            </Box>}
            {step === 'upgrade' && <Box>
                <Text color="white">HTTP → HTTPS upgrade? (y/n): </Text>
                <Text color={httpsUpgrade ? 'green' : 'red'}>{httpsUpgrade ? 'Yes' : 'No'}</Text>
            </Box>}
        </Box>
    );
}
...
function AddGroupForm({ onSave, onCancel, configMode }: AddGroupFormProps) {
    ...
    return (
        <Box flexDirection="column" paddingX={2} paddingY={1} borderStyle="round">
            <Text bold color="cyan">Add Route Group</Text>
            <Box>
                <Text color={step === 'name' ? 'white' : 'gray'}>Group Name: </Text>
                <Text>{name}{step === 'name' && <Text color="cyan">|</Text>}</Text>
            </Box>
            {step === 'source' && (
                <Box>
...
export function RouteConfig({ global, updateGlobal, activeGroups, onToggleGroup, onClose, configMode, taggedGroups }: RouteConfigProps) {
    ...
    for (const [name, group] of Object.entries(global.groups)) {
        ...
        for (let i = 0; i < group.routes.length; i++) {
            items.push({ type: 'route', groupName: name, index: i, route: group.routes[i]! });
        }
    ...
    if (mode === 'add-route') {
        return (
            <AddRouteForm
                ...
                onSave={(route) => {
                    const newGlobal = { ...global };
                    if (!newGlobal.groups[targetGroup]) newGlobal.groups[targetGroup] = { routes: [] };
    ...
    return (
        <Box flexDirection="column" borderStyle="round" paddingX={2} paddingY={1}>
            <Box flexDirection="row" justifyContent="space-between">
                <Text bold color="cyan">Route Configuration</Text>
                <Text color="gray">Port 80 enabled + HTTPS Upgrade</Text>
            </Box>
            <Box flexDirection="column" marginTop={1}>
                {items.length === 0 ? <Text color="gray">No items</Text> :
                    items.map((item, i) => {
                        const isSelected = i === selectedIndex;
                        const pointer = isSelected ? <Text color="cyan">{'> '}</Text> : <Text>{'  '}</Text>;

                        if (item.type === 'group') {
                            const isActive = activeGroups === undefined || activeGroups.includes(item.name);
                            const tagged = taggedGroups[item.name];
                            const sourcePrefix = configMode === 'merged' && tagged
                                ? <Text color={tagged.source === 'global' ? 'cyan' : 'yellow'}>{tagged.source}: </Text>
                                : null;
                            return (
                                <Box key={`group-${item.name}`} marginTop={i > 0 ? 1 : 0}>
                                    ...
                                    {sourcePrefix}
                                    <Text bold color={isSelected ? 'white' : (isActive ? 'white' : 'gray')}>{item.name}</Text>
                        ...
                        if (item.type === 'route') {
                            return (
                                <Box key={`route-${item.groupName}-${item.index}`} paddingLeft={4}>
                                    {pointer}
                                    <Box flexDirection="row" gap={1}>
                                        <Text color={isSelected ? 'white' : 'gray'} dimColor={!isSelected}>
                                            → <Text color={isSelected ? 'cyan' : undefined}>{item.route.domain}{item.route.path ?? ''}</Text>
                                            {' '}→ localhost:{item.route.target}
                                        </Text>
                                        {item.route.httpsUpgrade && <Text color="green">[HTTPS]</Text>}
                                        {item.route.latencyMs !== undefined && <Text color="yellow"> +{item.route.latencyMs}ms</Text>}
                                    </Box>
                                </Box>
                            );
                        }
                        if (item.type === 'add-route') {
                            return (
                                <Box key={`addroute-${item.groupName}`} paddingLeft={4}>
                                    {pointer}
                                    <Text color="gray" dimColor={!isSelected}>+ Add route...</Text>
                                </Box>
                            );
                        }
                        return (
                            <Box key="addgroup" marginTop={1}>
                                {pointer}
                                <Text color="gray" dimColor={!isSelected}>+ Add new group...</Text>
                            </Box>
                        );
                    })
                }
