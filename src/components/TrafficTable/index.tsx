...
import { formatLatency, formatMethod, formatRouteState, formatStatus } from '../../utils/format/index.js';
import type { TrafficTableProps } from './TrafficTable.types.js';

export function TrafficTable({ entries, selectedIndex, height, domains }: TrafficTableProps) {
    if (entries.length === 0) {
        const hint = domains && domains.length > 0
            ? `Try: curl https://${domains[0]}`
            : '';
        return (
    ...
    const startIdx = Math.max(0, Math.min(selectedIndex - Math.floor(visibleCount / 2), entries.length - visibleCount));
            ...
            {visible.map((entry, i) => {
                ...
                return (
                    <Box key={entry.id} flexDirection="row" gap={1} paddingX={1}>
                        ...
                        <Text color="white" bold={isSelected}>{entry.path}</Text>
